...
taxon_assign<- function(fileLoc = NULL, taxaDBLoc = NULL, numCores = 1, coverage = 95, ident = 95, propThres = 0.95, coverReportThresh=0, identReportThresh=0, includeAllDada=TRUE){
  ...
  if (is.null(fileLoc)){
    print(paste0("Select a file in the file folder with the BLAST output file(s) and associated Dada ASV output files (if present)."))
    fileLoc <- file.choose()
  }
  ...
  if(max(lengths(regmatches(fullPathFiles, gregexpr("_BLAST", fullPathFiles))))<=1){
    ...
    colnames(filesList) <- c("fileNames", "blastFilesFullPath", "blastFiles", "asvFilesFullPath", "asvFiles", "databaseName" )
