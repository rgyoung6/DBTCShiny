...
- <strong>numCores -</strong>  The combineReduceTaxa TRUE or FALSE value used to indicate if the read values should be replaced with presence/absence (1/0) data. This change is necessary when combining taxa for the same named samples across molecular markers but is not necessary when combining results for taxa with all unique sample names (default = TRUE to replace values with presence/absence 1/0 values). Note: For visulization on the DBTCShiny mapping component displaying read intensity all samples should have unique names and this value shoul dbe set to FALSE.
