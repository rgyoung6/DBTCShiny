...
#' revIdent = "_R2_001",nonMergeProcessing = TRUE, # compressValue = TRUE,
...
#' @param fwdIdent Foward identifier naming substring.
#' @param revIdent Reverse identifier naming substring.
#' @param nonMergeProcessing Non-Merge Processing (Default TRUE).
...
dada_implement <- function(runFolderLoc = NULL, primerFile = NULL,
  ...
  if(fwdPrimeMaxLen == 0 | revPrimeMaxLen == 0){
  ...
  if(is.null(runFolderLoc)){
    ...
    for(runCounter in 1:length(runFolderLoc)){
      ...
      suppressWarnings(write(paste0("Non-merge processing (nonMergeProcessing): ", nonMergeProcessing),file=paste0(outFolder,"/", dateStamp,dirName, "_dadaSummary.txt"),append = TRUE))
      ...
      for (listCounter in 1:length(fwdFile$fileNamesNoDir)){
        fwdInputTempPlotValues<-suppressWarnings(plotQualityProfile(fwdFile[listCounter,1])) # Forward
        ...
        ggsave(paste0(qualityFolder, "/", dateStamp, fwdFile[listCounter,4], "_fwdQual.pdf"))
      }
      for (listCounter in 1:length(revFile$fileNamesNoDir)){
        revInputTempPlotValues<-suppressWarnings(plotQualityProfile(revFile[listCounter,1])) # Reverse
        ...
        ggsave(paste0(qualityFolder, "/", dateStamp, revFile[listCounter,4], "_revQual.pdf"))
      }
      ...
      #Data frame with the files to be used in error checking
      errCheckFiles <- filesTbl[filesTbl$fileNamesNoDir %in% errCheckFiles,]

      #Print the error checking files to the screen and log file
      print(paste0("Here are the error checking files used..."))
      print(errCheckFiles$fileList)
      errCheckNote <- c(paste0("Here are the error checking files used..."), errCheckFiles$fileList)
      ...
      #Remove forward and reverse records and only include Merged results
      notShort<-notShort[notShort$Results == "Merged",, drop=FALSE]

      #add unique ids to the records
      notShort <- data.frame(uniqueID = paste0("M",c(1:nrow(notShort))), notShort, check.names=FALSE)
      ...
      #add unique ids to the records
      seqOut <- data.frame(uniqueID = paste0(">M",c(1:nrow(notShort))), notShort$Sequence, check.names=FALSE)
